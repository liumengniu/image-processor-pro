# Privacy Policy — Telegram™ Real-time Translation

**Last updated:** 2026-07-30

## Overview

Telegram™ Real-time Translation ("the Extension") is a browser extension that translates
messages on Telegram Web A (`https://web.telegram.org/a`) using the public
...
- Token returned from `tranai.app/api/login` (stored locally in
  `chrome.storage.local` as `telegrama_token`)
- Customer / account info returned by the backend (stored as `telegrama_customer`)
- Device ID (random UUID generated locally, stored as `telegrama_device_id`)
- Token expiration timestamp (stored as `telegrama_expiration_time`)
...
- Translation **cache** entries are stored in IndexedDB under the database
  name `telegrama_cache_db`, table `translations`
...
- User identity on Telegram (your TG account ID, username, phone number)
...
| Translation cache | IndexedDB (`telegrama_cache_db`) | No | Until cleared by user |
...
| `web.telegram.org` | `https://web.telegram.org/a` | Host page (read-only access) |

The Extension does **not** modify any data on `web.telegram.org`. It only
...
- **Clear translation cache** — clears all entries in `telegrama_cache_db`
...
`privacy@telegrama-translator.example`
...
endorsed by, or sponsored by Telegram FZ-LLC or WhatsApp LLC.** Telegram is a
trademark of Telegram FZ-LLC. WhatsApp is a trademark of WhatsApp LLC.
